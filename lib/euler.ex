defmodule Euler do
  ...
  def p11 do
    ...
    01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48
    """
    products(binary_to_matrix(grid), coordinates) |> Enum.max
  end

  def binary_to_matrix(binary) do
    String.split(binary, "\n", trim: true)
    |> Enum.map(&row_to_integers/1)
  end

  def row_to_integers(row) do
    String.split(row, " ") |> Enum.map(fn(x) -> elem(Integer.parse(x), 0) end)
  end
  ...
  # cred sorentwo for reduce usage
  def products(matrix, coords) do
    Enum.flat_map(coords, fn(pair) -> [vprod(pair, matrix), hprod(pair, matrix), drprod(pair, matrix), dlprod(pair, matrix)] end)
  end

  def vprod({x,y}, matrix) do
    Enum.reduce([0, 1, 2, 3], 1, fn(n, acc) -> Enum.at(Enum.at(matrix, x + n, []), y, 0) * acc end)
  end

  def hprod({x,y}, matrix) do
    Enum.reduce([0, 1, 2, 3], 1, fn(n, acc) -> Enum.at(Enum.at(matrix, x, []), y + n, 0) * acc end)
  end

  def drprod({x,y}, matrix) do
    Enum.reduce([0, 1, 2, 3], 1, fn(n, acc) -> Enum.at(Enum.at(matrix, x + n, []), y + n, 0) * acc end)
  end

  def dlprod({x,y}, matrix) do
    Enum.reduce([0, 1, 2, 3], 1, fn(n, acc) -> Enum.at(Enum.at(matrix, x + n, []), y - n, 0) * acc end)
  end
end
